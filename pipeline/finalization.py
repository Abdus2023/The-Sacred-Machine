...
def _paths(root: Path) -> dict[str, Path]:
    return {
        ...
        "certification_schema": root / "pipeline/schemas/certification.schema.json",
        "finalization_schema": root / "pipeline/schemas/finalization.schema.json",
...
def _revalidate_certificate_without_assembly(root: Path, evidence_class: str) -> dict[str, Any]:
    ...
    mapping_ok = isinstance(mapping_manifest, dict) and paths["mapping_input"].exists() and mapping_value_hash == bindings.get("mapping_sha256") and mapping_manifest.get("mapping_sha256") == mapping_value_hash and mapping_manifest.get("mapping_input_sha256") == sha256_file(paths["mapping_input"]) and mapping_manifest.get("status") == "AUTHORIZED" and mapping_manifest.get("authorization") == "AUTHORIZED" and mapping_manifest.get("source_manifest_hash") == source_manifest_hash and mapping_manifest.get("block_manifest_hash") == blocks_hash and mapping_manifest.get("outline_raw_sha256") == (outline or {}).get("raw_sha256") and mapping_manifest.get("outline_normalized_sha256") == (outline or {}).get("normalized_sha256") and mapping_manifest.get("outline_review_hash") == review_hash and mapping_manifest.get("review_status") == "REVIEWED" and mapping_manifest.get("mapping_validation_sha256") == (sha256_file(paths["mapping_validation"]) if paths["mapping_validation"].exists() else None)
    ...
    map000_ok = isinstance(map000, dict) and map000.get("status") == "PASS" and map000.get("source_manifest_hash") == source_manifest_hash and map000.get("block_manifest_hash") == blocks_hash and map000.get("outline_raw_sha256") == (outline or {}).get("raw_sha256") and map000.get("outline_normalized_sha256") == (outline or {}).get("normalized_sha256") and map000.get("outline_review_hash") == review_hash and map000.get("mapping_sha256") == mapping_value_hash
    ...
    assembly_ok = isinstance(assembly_manifest, dict) and assembly_manifest_hash == bindings.get("assembly_manifest_hash") and assembly_manifest.get("manifest_type") == "assembly" and assembly_manifest.get("status") == "VERIFIED" and assembly_manifest.get("mapping_status") == "AUTHORIZED" and assembly_manifest.get("assembly_schema_version") == "1.7.0" and assembly_manifest.get("pipeline_version") == PIPELINE_VERSION and assembly_manifest.get("source_representation") == "NORMALIZED_SOURCE" and assembly_manifest.get("candidate_sha256") == candidate_hash and assembly_manifest.get("assembly_sidecar_sha256") == (sha256_file(paths["assembly"]) if paths["assembly"].exists() else None) and assembly_manifest.get("dependencies") == expected_dependency and assembly_dependencies == expected_dependency and assembly_manifest.get("dependency_sha256") == sha256_bytes(canonical_json_bytes(expected_dependency))
