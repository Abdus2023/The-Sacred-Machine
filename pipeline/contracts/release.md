...
Revision 1.9 permits a separate `finalize` command only after FINAL-000 revalidates the current Revision 1.8 certificate and all dependency hashes. It performs an atomic binary-safe copy to `BOOK_FINAL.md`, verifies exact byte identity, and creates `pipeline/manifests/final.manifest.json`. It does not assemble, normalize, rewrite, or recertify.

Repository evidence and fixture evidence remain distinct. A fixture certificate and fixture final artifact cannot authorize repository release. Any stale or revoked review invalidates mapping, assembly, verification, certification, and finalization authorization. Final artifact tampering is detected by `verify-final`.
